...
async function printProfessional(title,folio,body,meta=""){
 ...
 .brandbar{height:9px;background:#173a61}.header{display:grid;grid-template-columns:1fr auto;gap:24px;align-items:center;padding:22px 26px 17px;border-bottom:1px solid #e2e8f0}.brandbox{display:flex;align-items:center;gap:15px}.logo{max-width:${Number(c.logoDocSize||145)}px;max-height:82px;object-fit:contain}.company-name{font-size:22px;font-weight:900;color:#173a61;letter-spacing:-.02em}.company-sub{color:#718096;font-size:10px;margin-top:3px}.header-info{text-align:right;color:#64748b;font-size:10px}.header-info strong{display:block;color:#173a61;font-size:11px}
...
let opProducts=[],opClients=[],opCategories=[],opCart=[],opFilter="all";
function opRecent(){try{return JSON.parse(localStorage.getItem("aura-op-recent")||"[]")}catch(e){return[]}}function opFreq(){try{return JSON.parse(localStorage.getItem("aura-op-freq")||"{}")}catch(e){return{}}}
async function loadOperationData(){let [p,c,k]=await Promise.allSettled([getDocs(collection(db,"productos")),getDocs(collection(db,"clientes")),getDocs(collection(db,"categorias"))]);opProducts=p.status==="fulfilled"?p.value.docs.map(d=>({id:d.id,...d.data()})).filter(x=>x.activo!==false):[];opClients=c.status==="fulfilled"?c.value.docs.map(d=>({id:d.id,...d.data()})):[];opCategories=k.status==="fulfilled"?k.value.docs.map(d=>({id:d.id,...d.data()})):[];$("#opClient").innerHTML='<option value="mostrador">👤 Venta de mostrador</option>'+opClients.map(x=>`<option value="${x.id}">${x.nombre||x.telefono} · ${x.telefono||""}</option>`).join("");$("#opCategory").innerHTML='<option value="">Todas</option>'+opCategories.filter(x=>x.activo!==false).map(x=>`<option value="${x.id}">${x.nombre}</option>`).join("");renderOpProducts()}
function renderOpProducts(){let q=$("#opSearch").value.toLowerCase(),cat=$("#opCategory").value,arr=opProducts.filter(p=>(!q||(p.nombre||"").toLowerCase().includes(q)||(p.sku||"").toLowerCase().includes(q)||(p.categoriaNombre||"").toLowerCase().includes(q))&&(!cat||p.categoriaID===cat||p.categoriaId===cat));if(opFilter==="stock")arr=arr.filter(p=>Number(p.stock||0)>0);if(opFilter==="recent"){let ids=opRecent();arr=arr.filter(p=>ids.includes(p.id)).sort((a,b)=>ids.indexOf(a.id)-ids.indexOf(b.id))}if(opFilter==="frequent"){let f=opFreq();arr=arr.filter(p=>f[p.id]).sort((a,b)=>(f[b.id]||0)-(f[a.id]||0))}$("#opProductList").innerHTML=arr.map(p=>`<div class="op-product"><img src="${driveImage(p.imagen||p.imageUrl||p.imagenUrl||"")}"><div><strong>${p.nombre}</strong><small>${p.sku||"Sin SKU"} · Stock ${Number(p.stock||0)}</small><div class="op-prices">Men. <b>${mx(p.precioMenudeo||p.precio||0)}</b>${Number(p.precioMayoreo||0)>0?` · May. <b>${mx(p.precioMayoreo)}</b> mín. 3`:""}</div></div><div class="op-adds"><button data-op-add="${p.id}" data-mode="menudeo">+ Men.</button>${Number(p.precioMayoreo||0)>0?`<button data-op-add="${p.id}" data-mode="mayoreo">+ May.</button>`:""}</div></div>`).join("")||'<p class="muted">Sin resultados.</p>';$$("[data-op-add]").forEach(b=>b.onclick=()=>addOp(b.dataset.opAdd,b.dataset.mode))}
function addOp(id,mode){let p=opProducts.find(x=>x.id===id),x=opCart.find(x=>x.id===id&&x.mode===mode),min=mode==="mayoreo"?3:1,price=mode==="mayoreo"?Number(p.precioMayoreo):Number(p.precioMenudeo||p.precio||0);if(x)x.qty++;else opCart.push({id,nombre:p.nombre,mode,qty:min,min,price});renderOpCart()}
function renderOpCart(){let total=opCart.reduce((s,x)=>s+x.qty*x.price,0),units=opCart.reduce((s,x)=>s+x.qty,0);$("#opCartItems").innerHTML=opCart.map((x,i)=>`<div class="op-cart-item"><div><strong>${x.nombre}</strong><b>${mx(x.qty*x.price)}</b></div><small>${x.mode==="mayoreo"?"Mayoreo":"Menudeo"} · ${mx(x.price)} c/u</small><div class="op-qty"><button data-minus="${i}">−</button><b>${x.qty}</b><button data-plus="${i}">+</button><button class="op-remove" data-remove="${i}">Eliminar</button></div></div>`).join("")||'<p class="muted">Agrega productos.</p>';$("#opUnits").textContent=units;$("#opTotal").textContent=mx(total);$("#opMobileUnits").textContent=units;$("#opMobileTotal").textContent=mx(total);$("#opContinue").disabled=!opCart.length;$$("[data-minus]").forEach(b=>b.onclick=()=>{let x=opCart[+b.dataset.minus];x.qty=Math.max(x.min,x.qty-1);renderOpCart()});$$("[data-plus]").forEach(b=>b.onclick=()=>{opCart[+b.dataset.plus].qty++;renderOpCart()});$$("[data-remove]").forEach(b=>b.onclick=()=>{opCart.splice(+b.dataset.remove,1);renderOpCart()});updateOpPayment()}
function updateOpPayment(){let total=opCart.reduce((s,x)=>s+x.qty*x.price,0),type=$("#opResult").value;if(type==="contado")$("#opInitialPayment").value=total.toFixed(2);let paid=type==="pedido"?0:Number($("#opInitialPayment").value||0);$("#opBalance").textContent="Saldo "+mx(Math.max(0,total-paid))}
async function registerOp(){let type=$("#opResult").value,total=opCart.reduce((s,x)=>s+x.qty*x.price,0),cid=$("#opClient").value,client=opClients.find(x=>x.id===cid),counter=cid==="mostrador",paid=type==="contado"?total:Number($("#opInitialPayment").value||0);if(type==="credito"&&counter)return $("#opMessage").textContent="Selecciona un cliente para venta a crédito.";if(paid<0||paid>total)return $("#opMessage").textContent="Abono inválido.";let products=opCart.map(x=>({productoID:x.id,nombre:x.nombre,modalidad:x.mode,cantidad:x.qty,precioUnitario:x.price,subtotal:x.qty*x.price}));try{$("#opContinue").disabled=true;if(type==="pedido"){let folio=`PED-M-${Date.now().toString().slice(-7)}`;await addDoc(collection(db,"pedidos"),{folio,clienteID:counter?null:client.id,clienteNombre:counter?"Venta de mostrador":client.nombre,telefono:counter?"":client.telefono||"",productos,total,estado:"nuevo",origen:"manual",createdAt:serverTimestamp(),updatedAt:serverTimestamp()});await log(`Creó pedido manual ${folio}`)}else{for(let x of opCart){let p=opProducts.find(z=>z.id===x.id);if(Number(p.stock||0)<x.qty)throw new Error(`Stock insuficiente: ${x.nombre}`)}let folio=`VTA-M-${Date.now().toString().slice(-7)}`,vr=doc(collection(db,"ventas"));await runTransaction(db,async tx=>{let ops=[];for(let x of opCart){let pr=doc(db,"productos",x.id),sn=await tx.get(pr),before=Number(sn.data().stock||0),after=before-x.qty;if(after<0)throw new Error(`Stock insuficiente: ${x.nombre}`);ops.push({pr,x,before,after,mr:doc(collection(db,"movimientosInventario"))})}tx.set(vr,{folio,clienteID:counter?null:client.id,clienteNombre:counter?"Venta de mostrador":client.nombre,telefono:counter?"":client.telefono||"",productos,total,totalPagado:paid,saldo:total-paid,tipo:type==="contado"?"contado":"credito",estado:total-paid<=0?"pagada":"pendiente",metodoPago:$("#opPaymentMethod").value,origen:"manual",createdAt:serverTimestamp(),updatedAt:serverTimestamp()});for(let z of ops){tx.update(z.pr,{stock:z.after,updatedAt:serverTimestamp()});tx.set(z.mr,{productoID:z.x.id,productoNombre:z.x.nombre,tipo:"venta",cambio:-z.x.qty,cantidad:z.x.qty,stockAnterior:z.before,stockNuevo:z.after,motivo:"Venta manual",referencia:folio,ventaID:vr.id,origen:"venta_manual",createdAt:serverTimestamp()})}});if(paid>0)await addDoc(collection(db,"abonos"),{ventaID:vr.id,ventaFolio:folio,clienteID:counter?null:client.id,clienteNombre:counter?"Venta de mostrador":client.nombre,monto:paid,metodo:$("#opPaymentMethod").value,origen:"venta_manual",createdAt:serverTimestamp(),usuario:auth.currentUser.uid});await log(`Registró venta manual ${folio}`)}let f=opFreq(),r=opRecent();opCart.forEach(x=>{f[x.id]=(f[x.id]||0)+x.qty;r=[x.id,...r.filter(y=>y!==x.id)].slice(0,12)});localStorage.setItem("aura-op-freq",JSON.stringify(f));localStorage.setItem("aura-op-recent",JSON.stringify(r));$("#opMessage").textContent="✓ Operación registrada.";setTimeout(()=>{$("#operationModal").classList.remove("open");refreshAllData(true)},700)}catch(e){$("#opMessage").textContent=e.message||"No fue posible registrar."}finally{$("#opContinue").disabled=false}}
$("#newOperationBtn").onclick=()=>{opCart=[];$("#operationModal").classList.add("open");document.querySelector(".operation-shell").classList.remove("mobile-cart-open");renderOpCart();loadOperationData()};$("#closeOperation").onclick=()=>$("#operationModal").classList.remove("open");$("#opSearch").oninput=renderOpProducts;$("#opCategory").onchange=renderOpProducts;$("#opResult").onchange=updateOpPayment;$("#opInitialPayment").oninput=updateOpPayment;$("#opContinue").onclick=registerOp;$("#opMobileCart").onclick=()=>document.querySelector(".operation-shell").classList.toggle("mobile-cart-open");$$(".op-filter").forEach(b=>b.onclick=()=>{$$(".op-filter").forEach(x=>x.classList.remove("active"));b.classList.add("active");opFilter=b.dataset.opFilter;renderOpProducts()});
...
function updateLogoSizeControls(){
 const w=Number($("#cfgLogoWebSize")?.value||160),m=Number($("#cfgLogoMobileSize")?.value||110),d=Number($("#cfgLogoDocSize")?.value||145);
 if($("#logoWebSizeValue"))$("#logoWebSizeValue").textContent=w;if($("#logoMobileSizeValue"))$("#logoMobileSizeValue").textContent=m;if($("#logoDocSizeValue"))$("#logoDocSizeValue").textContent=d;
 let preview=$("#logoSizePreview"),source=$("#logoPreview")?.src||"";if(preview){if(source){preview.src=source;preview.style.display="block";preview.style.width=Math.min(w,280)+"px"}else preview.style.display="none"}
}
async function loadLogoSizeConfig(){try{let sn=await getDoc(doc(db,"configuracion","empresa")),d=sn.exists()?sn.data():{};$("#cfgLogoWebSize").value=Number(d.logoWebSize||160);$("#cfgLogoMobileSize").value=Number(d.logoMobileSize||110);$("#cfgLogoDocSize").value=Number(d.logoDocSize||145);updateLogoSizeControls()}catch(e){}}
...
// v1.9.7.3 Document Center UI. Stable v1.9.6.2 login/auth lifecycle intentionally untouched.
let docsUI={tab:"ventas",days:"all",limit:20,selected:null};
function dcDate(v){return tsDate(v)} function dcText(v){let d=dcDate(v);return d?d.toLocaleDateString("es-MX",{day:"2-digit",month:"short",year:"numeric"}):"Sin fecha"}
function dcDateOk(v){if(docsUI.days==="all")return true;let d=dcDate(v);if(!d)return false,n=Number(docsUI.days);if(n===0)return d.toDateString()===new Date().toDateString();return Date.now()-d.getTime()<=n*86400000}
function dcSource(){let q=(document.querySelector("#docsSearch")?.value||"").toLowerCase().trim(),arr=[];if(docsUI.tab==="ventas")arr=docSales.filter(x=>dcDateOk(x.createdAt)).filter(x=>!q||`${x.folio||""} ${x.clienteNombre||""} ${x.telefono||""}`.toLowerCase().includes(q));if(docsUI.tab==="pedidos")arr=docOrders.filter(x=>dcDateOk(x.createdAt)).filter(x=>!q||`${x.folio||""} ${x.clienteNombre||""} ${x.telefono||""}`.toLowerCase().includes(q));if(docsUI.tab==="cuentas")arr=docClients.filter(x=>!q||`${x.nombre||""} ${x.telefono||""} ${x.correo||x.email||""}`.toLowerCase().includes(q));if(docsUI.tab==="kardex")arr=docProducts.filter(x=>!q||`${x.nombre||""} ${x.sku||""} ${x.categoriaNombre||""}`.toLowerCase().includes(q));return arr.sort((x,y)=>(dcDate(y.createdAt)||0)-(dcDate(x.createdAt)||0))}
function dcRender(){let box=document.querySelector("#docsResults");if(!box)return;let arr=dcSource(),shown=arr.slice(0,docsUI.limit),names={ventas:"ventas",pedidos:"pedidos",cuentas:"clientes",kardex:"productos"};document.querySelector("#docsListTitle").textContent=`${document.querySelector("#docsSearch").value?"Resultados":"Últimos"} ${Math.min(docsUI.limit,arr.length)} ${names[docsUI.tab]}`;document.querySelector("#docsResultCount").textContent=`${arr.length} encontrados`;document.querySelector("#docsLoadMore").classList.toggle("hidden",arr.length<=docsUI.limit);box.innerHTML=shown.map(x=>{let t,s,amt="";if(docsUI.tab==="ventas"){t=`${x.folio||"Venta"} · ${x.clienteNombre||"Mostrador"}`;s=`${dcText(x.createdAt)} · ${x.estado||x.tipo||""}`;amt=docMoney(x.total)}if(docsUI.tab==="pedidos"){t=`${x.folio||"Pedido"} · ${x.clienteNombre||"Sin cliente"}`;s=`${dcText(x.createdAt)} · ${x.estado||""}`;amt=docMoney(x.total)}if(docsUI.tab==="cuentas"){t=x.nombre||x.telefono||"Cliente";s=`${x.telefono||""}${x.correo||x.email?" · "+(x.correo||x.email):""}`}if(docsUI.tab==="kardex"){t=x.nombre||"Producto";s=`SKU ${x.sku||"—"} · Stock ${Number(x.stock||0)}`}return `<button class="doc-result ${docsUI.selected===x.id?"active":""}" data-dc-id="${x.id}"><span><strong>${escDoc(t)}</strong><small>${escDoc(s)}</small></span><span class="doc-amount">${amt}</span></button>`}).join("")||'<p class="muted">No encontramos registros.</p>';document.querySelectorAll("[data-dc-id]").forEach(b=>b.onclick=()=>{docsUI.selected=b.dataset.dcId;dcRender();dcDetail()})}
function dcDetail(){let x=dcSource().find(v=>v.id===docsUI.selected),box=document.querySelector("#docsDetail");if(!box)return;if(!x){box.innerHTML='<div class="docs-empty"><span>📄</span><strong>Selecciona un registro</strong><p>Revisa la información antes de generar el documento.</p></div>';return}if(docsUI.tab==="ventas"||docsUI.tab==="pedidos"){let lines=(x.productos||[]).map(i=>`<div class="doc-line"><span><strong>${escDoc(i.nombre||"Producto")}</strong><small>${Number(i.cantidad||1)} × ${docMoney(i.precioUnitario)}</small></span><strong>${docMoney(i.subtotal||Number(i.cantidad||1)*Number(i.precioUnitario||0))}</strong></div>`).join("");box.innerHTML=`<div class="doc-sheet-head"><div><small>${docsUI.tab==="ventas"?"VENTA":"PEDIDO"}</small><h2>${escDoc(x.folio||"Sin folio")}</h2></div><span>${escDoc(x.estado||x.tipo||"")}</span></div><div class="doc-info-grid"><div class="doc-info"><small>Cliente</small><strong>${escDoc(x.clienteNombre||"Mostrador")}</strong><span>${escDoc(x.telefono||"")}</span></div><div class="doc-info"><small>Fecha</small><strong>${dcText(x.createdAt)}</strong><span>Origen: ${escDoc(x.origen||"sistema")}</span></div></div>${lines}<div class="doc-info"><small>Total</small><strong>${docMoney(x.total)}</strong>${docsUI.tab==="ventas"?`<span>Pagado ${docMoney(x.totalPagado)} · Saldo ${docMoney(x.saldo)}</span>`:""}</div><div class="doc-actions"><button class="btn btn-primary" id="dcGenerate">Generar documento</button></div>`;document.querySelector("#dcGenerate").onclick=()=>{if(docsUI.tab==="ventas"){document.querySelector("#docSaleSelect").value=x.id;generateSaleDoc()}else{document.querySelector("#docOrderSelect").value=x.id;generateOrderDoc()}}}if(docsUI.tab==="cuentas"){box.innerHTML=`<div class="doc-sheet-head"><div><small>CLIENTE</small><h2>${escDoc(x.nombre||x.telefono)}</h2></div></div><div class="doc-info-grid"><div class="doc-info"><small>Teléfono</small><strong>${escDoc(x.telefono||"—")}</strong></div><div class="doc-info"><small>Correo</small><strong>${escDoc(x.correo||x.email||"—")}</strong></div><div class="doc-info"><small>Total comprado</small><strong>${docMoney(x.totalComprado)}</strong></div><div class="doc-info"><small>Saldo pendiente</small><strong>${docMoney(x.totalPendiente)}</strong></div></div><div class="doc-actions"><button class="btn btn-primary" id="dcGenerate">Generar estado de cuenta</button></div>`;document.querySelector("#dcGenerate").onclick=()=>{document.querySelector("#docClientSelect").value=x.id;generateClientDoc()}}if(docsUI.tab==="kardex"){let moves=docInventory.filter(m=>m.productoID===x.id).sort((a,b)=>(dcDate(b.createdAt)||0)-(dcDate(a.createdAt)||0)).slice(0,50);box.innerHTML=`<div class="doc-sheet-head"><div><small>KARDEX</small><h2>${escDoc(x.nombre)}</h2></div><span>Stock ${Number(x.stock||0)}</span></div><div style="overflow:auto"><table class="kardex-table"><thead><tr><th>Fecha</th><th>Tipo</th><th>Cambio</th><th>Existencia</th></tr></thead><tbody>${moves.map(m=>`<tr><td>${dcText(m.createdAt)}</td><td>${escDoc(m.tipo||"")}</td><td>${Number(m.cambio||0)}</td><td>${Number(m.stockNuevo??0)}</td></tr>`).join("")||'<tr><td colspan="4">Sin movimientos.</td></tr>'}</tbody></table></div><div class="doc-actions"><button class="btn btn-primary" id="dcGenerate">Generar Kardex</button></div>`;document.querySelector("#dcGenerate").onclick=()=>{document.querySelector("#docProductSelect").value=x.id;generateKardexDoc()}}}
document.querySelectorAll(".docs-tab").forEach(b=>b.onclick=()=>{document.querySelectorAll(".docs-tab").forEach(x=>x.classList.remove("active"));b.classList.add("active");docsUI.tab=b.dataset.docTab;docsUI.limit=20;docsUI.selected=null;document.querySelector("#docsSearch").value="";dcRender();dcDetail()});document.querySelectorAll(".docs-filter").forEach(b=>b.onclick=()=>{document.querySelectorAll(".docs-filter").forEach(x=>x.classList.remove("active"));b.classList.add("active");docsUI.days=b.dataset.docDays;docsUI.limit=20;dcRender()});let dcTimer;document.querySelector("#docsSearch").oninput=()=>{clearTimeout(dcTimer);dcTimer=setTimeout(()=>{docsUI.limit=20;docsUI.selected=null;dcRender();dcDetail()},180)};document.querySelector("#docsLoadMore").onclick=()=>{docsUI.limit+=20;dcRender()};
// Render after the proven legacy loader fills the arrays. This does not touch authentication.
setTimeout(dcRender,2300);
