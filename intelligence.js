...
function renderButtons(){
 let el=$("#aiQuickQuestions"); if(!el)return;
 el.innerHTML=groups.map(([title,ids])=>`<div class="ai-category"><h4>${title}</h4><div class="ai-category-buttons">${ids.map(i=>`<button type="button" class="ai-q" data-ai="${i}"><span class="ai-q-icon">${Q[i][0]}</span><span>${Q[i][1]}</span></button>`).join("")}</div></div>`).join("");
 el.querySelectorAll(".ai-q").forEach(b=>b.addEventListener("click",()=>show(Number(b.dataset.ai),b)));
}
function show(i,b){
 document.querySelectorAll("#asistente .ai-q").forEach(x=>x.classList.remove("selected"));b.classList.add("selected");
 $("#aiResultSubtitle").textContent=Q[i][1];let r=$("#aiResult");r.className="ai-result-content";r.innerHTML=`<h2>${Q[i][1]}</h2><div class="answer-box">${answer(i)}</div>`;
}
async function load(){
 renderButtons();
 let defs=[["sales","ventas"],["orders","pedidos"],["products","productos"],["clients","clientes"],["payments","abonos"],["tasks","tareasCRM"],["inventory","movimientosInventario"]];
 ...
 [["Sales","sales"],["Orders","orders"],["Products","products"],["Clients","clients"],["Payments","payments"],["Tasks","tasks"]].forEach(([id,k])=>{let e=$("#aiData"+id);if(e)e.textContent=D[k].length});
 let sync=$("#aiLastSync"),dot=$("#aiSyncDot");if(sync)sync.textContent=bad.length?"Carga parcial: "+bad.join(", "):"Actualizado · "+new Date().toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"});if(dot)dot.style.color=bad.length?"#f59e0b":"#22c55e";
}
